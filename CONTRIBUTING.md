...
- 索引页须提供到所有内容页的链接。
- 内容页间通过交叉引用建立知识网络。
- **作品大标题栏（左上角入口，必备）**：每部作品的每个内容页（**该作品首页本身除外**，首页可选保留）左上角必须放置一个醒目的「大标题」作为品牌入口——
  - 点击回到**该作品手册的 `index.html`**（**不是**项目根 `index.html`），与右下角指向**项目主页**的全局按钮职责分离（详见 [§7](#7-_shared-全局注入说明必读)、[ARCHITECTURE §4.3](./ARCHITECTURE.md#43-作品大标题栏per-work非自动注入)）。
  - 视觉重量须足以一眼识别为**品牌标识**而非辅助元素——粗体、对比色、独立区域等均可；具体字号与排版按「一作一貌」自主决定。
  - **同一作品的所有页面之间须视觉一致**（同 class、同字号、同 hover 行为）。
  - 该栏**写在每页 HTML 中**（一般放在 `<header class="site-header">` 区块），由各作品维护者手写——**不由 CI 注入**，便于本地预览即生效与跨作品差异化。
- **移动端导航收纳**：手机端（`< 768px`）须将顶部导航条、侧边目录、上一章/下一章控件等收纳为汉堡菜单或折叠面板，避免水平滚动；具体收纳方式按内容自定（详见 [§6.4 响应式设计要求](#64-响应式设计要求)）。
...
- **大标题栏样式**：作品大标题栏（左上角入口，详见 [§4.4](#44-导航要求)）的 class 名、字号、配色、hover 行为由各作品自由决定，但必须在该作品的**所有页面之间视觉一致**。建议通过 `:root` CSS 变量集中管理（如 `--title-color`、`--title-weight`），便于后续微调。

### 6.4 响应式设计要求

> 🟠 **与「一作一貌」并不矛盾**：**断点统一**（保证可读性），但每部作品在每档断点下的**布局形态**自由设计。

#### 6.4.1 推荐三档断点

| 档位 | 宽度 | 设计要点 |
|------|------|----------|
| **桌面端** | `≥ 1024px` | 完整布局得以展开——多栏对照、侧边目录、复杂图表等充分呈现 |
| **平板端** | `768–1023px` | 简化布局：合并次要栏（如"小说+理论"合并为一栏），主要栏保留独立 |
| **手机端** | `< 768px` | 单栏堆叠；复杂对照通过**标签页 / 折叠 / 汉堡菜单 / 锚点跳转**等机制切换，**避免横向滚动** |

- **断点可视作品性质微调**（插画型作品可能选用 1200 / 900；时间线型作品可能强调 600 中间档）——但每部作品必须有 ≥ 2 档清晰区分，保证手机端可正常阅读。
- 实现方式：`@media (min-width: 768px)` / `@media (min-width: 1024px)`（推荐移动优先 + 渐进增强）；单位优先 `rem` / `%` / `clamp()`，避免硬编码像素。

#### 6.4.2 典型示例：三栏对照布局（适用于「虚构 ↔ 学术 ↔ 历史」型作品）

> ⚠️ 此为**模板示例**，适用于采用三方对照结构（参见 §2.3 推荐的"手册 / 学术"布局）的作品；其他内容类型（全鉴 / 年表 / 地图）请按各自内容形态自定。

| 档位 | 三栏对照布局形态 |
|------|------------------|
| 桌面端 `≥ 1024px` | **三栏并排**：小说（虚构内容）30% + 学术理论 35% + 真实历史 35% |
| 平板端 `768–1023px` | **两栏并排**：小说 + 学术理论合并为左栏（约 60%）；真实历史栏下移至右栏下方 |
| 手机端 `< 768px` | **单栏堆叠**，通过**标签页**（小说 / 理论 / 历史）切换 |

#### 6.4.3 不同内容类型的响应式形态示例

| 内容类型 | 桌面端典型形态 | 手机端典型形态 |
|----------|----------------|----------------|
| **手册 / 学术**（三方对照） | 三栏并排（30/35/35） | 单栏 + 标签页切换 |
| **全鉴 / 图鉴**（料理番等） | 4 列卡片网格 + 搜索框置顶 | 单列瀑布流 + 搜索框置顶 + 分类筛选折叠 |
| **年表** | 垂直时间线（左中右三栏对照） | 垂直时间线（单栏堆叠）+ 年代筛选下拉 |
| **地图 / 地理** | 交互式地图 + 标注信息卡（侧边） | 地图占满 + 标注信息卡改为底部抽屉 |

> 上表仅作示意，**不强制套用**——具体形态由作品按内容真实需要设计。

#### 6.4.4 实现要点

- **单位与单位制**：长度单位优先 `rem` / `%` / `clamp()`；字号可用 `clamp()` 做流式排版（如 `font-size: clamp(1rem, 0.8rem + 1vw, 1.5rem)`）。
- **多栏对照**：用 CSS Grid（如 `grid-template-columns: 30% 35% 35%`）比 Float 更可控；平板合并可用 `grid-template-columns: 1fr` + 调整 row。
- **手机端标签页**：用 `<input type="radio">` + `:checked` 伪类实现无 JS 标签页，或用 5~10 行原生 JS 切换 `.is-active` class。
- **导航 / TOC 收纳**：手机端导航、长 TOC 在 `< 768px` 时收纳为汉堡菜单（与 [§4.4 作品大标题栏](#44-导航要求) 区域的 nav-toggle 联动即可）。
- **图表响应式**（[§5.3](#53-使用规范)）：Mermaid / ECharts 在 `< 768px` 时建议关闭动画、提供折叠控制，避免在小屏上视觉过载；图表字体大小可随断点缩放。
- **大表格 / 对照表**：在 `< 768px` 时考虑折叠为**卡片式**或**横向滚动容器**（带明确视觉提示，避免隐藏溢出）。
- **断点测试**：每次新增 / 修改内容页，**至少在桌面（1440px）、平板（800px）、手机（375px）三档各预览一次**，确保无横向滚动、无错位、可阅读。

#### 6.4.5 反模式

- ❌ 桌面端三栏但手机端仍是三栏硬挤——必须分档设计。
- ❌ 单纯用 `transform: scale()` 整体缩放——分档重排 > 整体缩放。
- ❌ 移动端禁用横向滚动却仍水平溢出——内容必须在容器内收敛。
- ❌ 只考虑桌面端，手机端 "差不多就行"——手机端是很多读者的主要阅读场景。
...
> ⚠️ **本节只覆盖右下角「返回主页」按钮。** 左上角的**「作品大标题」**栏（详见 [§4.4](#44-导航要求)）**不由 CI 注入**——它直接写在每页 HTML 中，由各作品维护者自行编写。因此本地预览作品页时，大标题会正常显示；只有右下角的「返回主页」按钮依赖 CI 注入，本地打开文件时不会出现（不影响阅读）。
...
- [ ] 展示作品概览与统计
- [ ] 提供到所有内容页的导航
- [ ] （可选）在首页也放置作品大标题栏，与内页保持一致；首页本身是否保留该栏由作者决定（详见 [§4.4](#44-导航要求)）

### Step 7.5 — 部署作品大标题栏（贯穿全部页面）

> 该步骤贯穿 Step 5 ~ Step 7 的所有页面编写；这里汇总检查项，确保成品每页都有。

- [ ] 在每页 HTML（`vol-*.html` / `arc-*.html` / `glossary.html` / `references.html` / `index.html` 等）的左上角放置醒目的「大标题」入口
- [ ] `<a href="index.html">` 链接到该作品的 `index.html`（**不是**项目根 `index.html`——见 [§4.4](#44-导航要求)）
- [ ] 视觉重量足以一眼识别为品牌标识（粗体 / 对比色 / 独立区域 / 与正文明显区分），但具体形态按「一作一貌」自定
- [ ] 在该作品**所有页面之间视觉一致**（同 class、同字号、同 hover 行为）
- [ ] 作品首页本身可选保留（首页即为目标页，可不显示）
- [ ] 大标题栏的 HTML 写在每页 HTML 中（一般放在 `<header class="site-header"> 区块`），**不由 CI 注入**——避免与 [§7](#7-_shared-全局注入说明必读) 中「不要手动写入 home-button 引用」混淆
...
- [ ] 本地静态服务器预览，所有页面正常
- [ ] **响应式三档断点检查**（桌面 ≥ 1024px / 平板 768–1023px / 手机 < 768px 三档各预览一次，无横向滚动、无错位、关键内容可阅读——详见 [§6.4](#64-响应式设计要求)）
- [ ] 所有链接有效
...
## 导航自检（新增作品必填）
- [ ] 所有内容页（除该作品首页）左上角放置醒目的「作品大标题」入口
- [ ] 大标题链接到该作品手册的 `index.html`（**不是**项目根 `index.html`）
- [ ] 大标题在该作品所有页面之间视觉一致
- [ ] 大标题栏直接写在 HTML 中，不由 CI 注入
- [ ] 手机端（< 768px）导航 / 侧边目录已收纳为汉堡菜单或折叠面板

## 响应式自检（新增作品必填）
- [ ] 桌面（≥ 1024px）/ 平板（768–1023px）/ 手机（< 768px）三档各预览一次
- [ ] 手机端无横向滚动，关键内容可阅读
- [ ] 复杂对照在手机端已折叠 / 合并 / 标签页切换
- [ ] 已按本作品内容类型确定对照形态（**非所有作品都套用「三方对照」**——料理番、年表、地图可采用不同形态）
...
| 作品 | 标题字体 | 正文字体 | 主色 | 色调 | 页面骨架 | 布局 | 交互 | 顶部大标题 | 响应式形态 |
|------|----------|----------|------|------|----------|------|------|------------|-------------|
| 魔王勇者 | Lora | Work Sans | `#8B0000` | 冷学术 | header→hero→main→footer | 卡片网格 + 左侧 TOC | 筛选/折叠/TOC/返回顶部 | `.site-logo`（字号偏小，**待强化**） | 桌面三栏对照（30/35/35），手机单栏 + 标签页——已实现，**作为模板示例** |
| 食戟之灵 | Lora | Work Sans | `#C0392B` | 暖料理 | header→hero→main→footer | 卡片网格 + 右侧 TOC | 筛选/折叠/TOC/返回顶部 | `.site-logo`（字号偏小，**待强化**） | 桌面卡片网格，手机单列瀑布流——**未严格遵循三栏对照**（全鉴类作品按内容自定） |

> 🔴 上表可见两部作品在字体、骨架、交互上**高度同质**。新作品**必须**打破这一模式：字体组合与主色须不同，骨架/布局/交互鼓励不同。
>
> 🟡 此外，两部作品的顶部入口**目前仅为普通尺寸的 logo**（1.1~1.25rem），尚未达到 [§4.4](#44-导航要求) 中「醒目大标题」的视觉重量；后续需按各自作品气质强化（粗体、放大字号、增强对比色等），同时保持作品内各页一致。新作品**不应沿用**这一尺寸——按 [§4.4](#44-导航要求) 直接以醒目大标题实现。
>
> 🟢 **响应式形态不要求统一**：上表中魔王勇者采用「三栏对照」是其内容形态（手册类）的合理选择；食戟之灵采用「卡片网格」则符合全鉴类作品的特点。新作品应按自身内容类型选择最合适的对照形态（参考 [§6.4.3](#643-不同内容类型的响应式形态示例)），**不必照搬三栏对照**。
