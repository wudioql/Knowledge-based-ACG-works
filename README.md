...
> 从动漫与游戏中挖掘真实世界知识，**按作品气质**建立虚构内容与现实知识的多维度对照（如学术型作品的「**虚构 ↔ 学术理论 ↔ 真实历史**」三方对照、料理番的「料理场景 ↔ 料理科学 ↔ 文化源流」、年表的「事件 ↔ 年代 ↔ 史实」等——**不强制所有作品都套用同一形态**）。
...
许多作品在虚构叙事中融入了真实的知识体系——经济学原理、政治制度、料理科学、历史事件。本项目将这些隐藏的知识脉络梳理出来，按作品气质建立不同形态的多维度对照，让作品成为通往真实知识世界的入口。
...
> *上图仅为学术型作品（手册类）的「**虚构 ↔ 学术 ↔ 历史**」三方对照示例。**不同内容类型可有完全不同的对照形态**：料理番对应「料理场景 ↔ 料理科学 ↔ 文化源流」、年表对应「事件 ↔ 年代 ↔ 史实」、地图对应「地理设定 ↔ 现实地理 ↔ 历史沿革」等。各作品按自身真实需要设计，详见 [CONTRIBUTING.md §2.3](./CONTRIBUTING.md#23-可选的差异化方向)。*
...
> 唯一的"构建"动作是 CI 在部署期自动注入右下角「返回主页」按钮（一段轻量 `find + sed` 处理，见 [ARCHITECTURE.md — 构建与部署流程](./ARCHITECTURE.md#5-构建与部署流程)）。
> 与之对应，**左上角的「作品大标题」**直接写在每页 HTML 中（详见 [CONTRIBUTING.md — 导航要求](./CONTRIBUTING.md#44-导航要求)），由各作品维护者自行编写，**不由 CI 注入**——因此本地直接打开作品页时：
> - ✅ **作品大标题会正常显示**（品牌入口可用）
> - ❌ **右下角「返回主页」按钮不会出现**（基本不影响阅读，仅不便切换作品，可回退至首页再切换）
>
> 本地预览推荐用下面的静态服务器，以保证相对路径与字体加载行为与线上一致。

---

## 项目特性

- **内容对照体系**：每部作品按自身气质建立虚构内容与现实知识的多维度对照（如学术型：虚构 ↔ 学术 ↔ 历史；料理番：料理场景 ↔ 料理科学 ↔ 文化源流；详见 [CONTRIBUTING §2.3](./CONTRIBUTING.md#23-可选的差异化方向)）——**不强制所有作品套用同一形态**
...
- **作品大标题栏**：每部作品的页面（**该作品首页本身除外**，首页可选保留）左上角设有醒目的「大标题」入口，点击可回到**该作品手册**的主页——区别于右下角指向**项目**主页的全局按钮。**由各作品维护者手写在每页 HTML 中**，不由 CI 注入，详见 [CONTRIBUTING.md — 导航要求](./CONTRIBUTING.md#44-导航要求)
- **全局返回按钮**：`_shared/` 在构建时自动为所有非首页页面注入右下角「返回主页」按钮（指向**项目**主页），**作品手册编写时无需关心**
- **自动部署**：推送至 GitHub 后自动部署到 GitHub Pages
- **多端响应式设计**：所有作品页适配**推荐三档断点**——桌面（≥ 1024px）/ 平板（768–1023px）/ 手机（< 768px），断点可视作品性质微调；具体布局形态按内容类型自定，详见 [CONTRIBUTING §6.4](./CONTRIBUTING.md#64-响应式设计要求)
...
| 样式 | CSS3 | CSS 变量 + 原生选择器，无预处理器；**推荐三档响应式断点**（≥ 1024px / 768–1023px / < 768px，可微调） |
...
| 类型 | 说明 | 推荐视觉模式 | 对照形态 | 示例 |
|------|------|--------------|----------|------|
| 手册 | 按章节系统梳理知识点，附学术对照 | 学术风格，双栏/三栏对照表，侧边目录 | 虚构 ↔ 学术 ↔ 历史（**三方对照**示例） | 魔王勇者 |
| 全鉴 | 逐条目详解，附技术拆解 | 图文并茂，卡片流，沉浸式排版 | 料理场景 ↔ 料理科学 ↔ 文化源流（**三方或双方对照**） | 食戟之灵 |
| 年表 | 按时间线整理事件与史实对照 | 垂直时间线，节点式布局 | 事件 ↔ 年代 ↔ 史实 | *(预留)* |
| 地图 | 地理设定与真实地理 / 历史对照 | 交互式地图，标注式信息卡 | 地理设定 ↔ 现实地理 ↔ 历史沿革 | *(预留)* |

> ⚠️ **对照形态不强制为"三方"**：每部作品按内容真实需要设计（可以是三方、双方、单方+注释等）。上表中的对照形态是各类型的**推荐起点**，详见 [CONTRIBUTING §6.4 响应式设计要求](./CONTRIBUTING.md#64-响应式设计要求)。
